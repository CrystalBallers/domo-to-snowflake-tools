...
# Use internal DomoHandler for authentication (aligned with main.py)
...
DEFAULT_CONFIG = {
    'domo_dataset_id': 'afeb261b-3270-409d-80d4-475a8c570940',  # Domo dataset ID
    'snowflake_table': 'stg_meltables_2025',       # Snowflake table name
    'key_columns': ['"ASIN"'],        
    ...
    'transform_column_names': False             # True = apply column name transformation, False = use original names
}
...
class SnowflakeConnector:
    ...
    def get_sample_data(self, table_name: str, key_columns: List[str], limit: int = 1000) -> pd.DataFrame:
        ...
        # Apply TRIM() to key columns for ordering
        trimmed_key_columns = [f"TRIM({col})" for col in processed_key_columns]
        key_cols_str = ', '.join(trimmed_key_columns)
...
class DatasetComparator:
    ...
    def __init__(self):
        self.domo = DomoHandler()
        # Authenticate using the same mechanism as main.py
        if not self.domo.setup_auth():
            logger.error("Failed to authenticate with Domo. Please check your environment variables.")
    ...
    def compare_schemas(self, domo_dataset_id: str, snowflake_table: str, transform_names: bool = False) -> Dict[str, Any]:
        ...
        try:
            # Get Domo schema (may fail for some dataset types, so add a fallback)
            domo_columns = {}
            try:
                domo_schema = self.domo.get_dataset_schema(domo_dataset_id)
                if isinstance(domo_schema, dict) and domo_schema.get('columns'):
                    domo_columns = {col['name']: col.get('type', 'STRING') for col in domo_schema['columns']}
                else:
                    logger.warning("Domo schema did not contain column metadata – falling back to sample extraction")
            except Exception as schema_err:
                logger.warning(f"Primary schema fetch failed: {schema_err}. Falling back to sample extraction")

            # Fallback ‑ use a 1-row sample to infer column names if schema is empty
            if not domo_columns:
                sample_df = self.domo.extract_data(
                    dataset_id=domo_dataset_id,
                    query="SELECT * FROM table LIMIT 1",
                    enable_auto_type_conversion=False
                )
                if sample_df is not None and len(sample_df.columns) > 0:
                    domo_columns = {col: 'STRING' for col in sample_df.columns}
                    logger.info(f"Derived {len(domo_columns)} columns from sample extraction")
                else:
                    raise Exception("Could not determine Domo columns via schema or sample extraction")
    ...
    def compare_row_counts(self, domo_dataset_id: str, snowflake_table: str) -> Dict[str, Any]:
        ...
        try:
            ...
            domo_count = int(domo_result['rows'][0][0]) if domo_result['rows'] else 0
        except Exception as e:
    ...
    def compare_data_samples(self, domo_dataset_id: str, snowflake_table: str, 
        ...
        try:
            domo_count_query = "SELECT COUNT(*) as row_count FROM table"
            domo_result = self.domo.query_dataset(domo_dataset_id, domo_count_query)
            total_domo_rows = int(domo_result['rows'][0][0]) if domo_result['rows'] else 0
        ...
        # Get Domo sample
        try:
            # Apply TRIM() to key columns for ordering in Domo query
            trimmed_key_columns = [f"TRIM({col})" for col in key_columns]
            key_cols_str = ', '.join(trimmed_key_columns)
        ...
        # Get total Snowflake count
        try:
            total_snowflake_rows = self.snowflake.get_row_count(snowflake_table)
            logger.info(f"Total Snowflake rows: {total_snowflake_rows}")
        except Exception as e:
            self.add_error("Total Snowflake Count", f"Could not get total Snowflake row count", str(e))
        ...
        try:
            ...
            detailed_report = self.generate_detailed_row_comparison(domo_df, sf_df, key_columns, total_domo_rows, total_snowflake_rows)
            logger.info("Detailed report generated")
            
            # Save detailed report to file
            timestamp = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
            log_filename = f"{snowflake_table}_{timestamp}.txt"
    ...
    def _compare_dataframes(self, domo_df: pd.DataFrame, sf_df: pd.DataFrame, 
        ...
        # Handle null/empty values consistently and standardize numeric keys
        def clean_key_value(val):
            if pd.isna(val) or str(val).strip() == '' or str(val).strip().lower() == 'null':
                return 'NULL'
            
            # Try to convert to integer if it's a numeric value
            val_str = str(val).strip()
            try:
                # Handle float strings like '10273.0' -> convert to int
        ...
        # Create composite keys - handle single vs multiple columns properly
        if len(processed_key_columns) == 1:
            # Single column case
            col = processed_key_columns[0]
            domo_df['_key'] = domo_df[col].apply(clean_key_value)
            sf_df['_key'] = sf_df[col].apply(clean_key_value)
        else:
            # Multiple columns case
            domo_df['_key'] = domo_df[processed_key_columns].apply(lambda row: '|'.join(clean_key_value(val) for val in row), axis=1)
            sf_df['_key'] = sf_df[processed_key_columns].apply(lambda row: '|'.join(clean_key_value(val) for val in row), axis=1)
    ...
    def generate_report(self, domo_dataset_id: str, snowflake_table: str, 
        ...
        data_comparison = self.compare_data_samples(domo_dataset_id, snowflake_table, key_columns, sample_size, transform_names, schema_comparison)
    ...
    def generate_detailed_row_comparison(self, domo_df: pd.DataFrame, sf_df: pd.DataFrame, 
                                       key_columns: List[str], total_domo_rows: int = None, 
                                       total_snowflake_rows: int = None) -> str:
        """
        ...
        # Add total row count information if available
        if total_domo_rows is not None and total_snowflake_rows is not None:
            report_lines.append(f"\n📈 TOTAL ROW COUNTS (FULL DATASETS):")
            report_lines.append(f"   Total rows in Domo (full): {total_domo_rows:,}")
            report_lines.append(f"   Total rows in Snowflake (full): {total_snowflake_rows:,}")
            report_lines.append(f"   Difference: {total_snowflake_rows - total_domo_rows:,}")
            
            # Check if missing rows might be due to size difference
            missing_in_sf = domo_keys - sf_keys
            extra_in_sf = sf_keys - domo_keys
            
            if total_snowflake_rows > total_domo_rows and missing_in_sf:
                report_lines.append(f"\n⚠️  SAMPLE SIZE ANALYSIS:")
                report_lines.append(f"   Snowflake has {total_snowflake_rows - total_domo_rows:,} more rows than Domo")
                report_lines.append(f"   Missing {len(missing_in_sf)} rows in Snowflake sample")
                report_lines.append(f"   This could be due to:")
                report_lines.append(f"   - Sample size limitations (only {len(sf_df)} rows sampled)")
                report_lines.append(f"   - The missing rows exist in Snowflake but not in this sample")
                report_lines.append(f"   - Need to investigate why Snowflake has more data than Domo")
                report_lines.append(f"   - Consider running a full comparison or larger sample")
            
            if total_domo_rows > total_snowflake_rows and extra_in_sf:
                report_lines.append(f"\n⚠️  SAMPLE SIZE ANALYSIS:")
                report_lines.append(f"   Domo has {total_domo_rows - total_snowflake_rows:,} more rows than Snowflake")
                report_lines.append(f"   Found {len(extra_in_sf)} extra rows in Snowflake sample")
                report_lines.append(f"   This could be due to:")
                report_lines.append(f"   - Sample size limitations (only {len(domo_df)} rows sampled)")
                report_lines.append(f"   - The extra rows exist in Domo but not in this sample")
                report_lines.append(f"   - Need to investigate why Domo has more data than Snowflake")
                report_lines.append(f"   - Consider running a full comparison or larger sample")
...
def trim_key_columns(key_columns: List[str]) -> List[str]:
    """Trim whitespace from key column names"""
    return [col.strip() for col in key_columns]
...
def get_config_from_args():
    ...
    # Apply trimming to key columns
    config['key_columns'] = trim_key_columns(config['key_columns'])
