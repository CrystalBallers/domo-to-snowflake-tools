...
================================================================================
Dataflow ID: 105
Dataflow Name: Sales Archive DF
...
TRANSLATION SUMMARY:
  ...
  Action Types: Constant, ExpressionEvaluator, Filter, GroupBy, LoadFromVault, MergeJoin, Metadata, PublishToVault, SelectValues, UnionAll
  Pipelines: 6

Generated: 2025-07-02 10:24:43
================================================================================
*/
WITH _countries AS (
  SELECT
  ...
  SELECT
    ...
    start_week
  FROM Dates_DS
), _View_of_sales_by_products_archive AS (
  SELECT
    country_id,
    product_id,
    amazon_seller_id,
    ...
    batch_id,
    batch_last_run
  FROM View_of_sales_by_products_archive
), _amazon_skus AS (
  SELECT
    amazon_asin_id,
    amazon_seller_id,
    ...
    batch_last_run
  FROM amazon_skus
), _View_of_amazon_order_items AS (
  SELECT
    asin,
    currency,
    id,
    ...
    batch_id,
    batch_last_run
  FROM View_of_amazon_order_items
), _View_of_Historical_Sales_Archive_DS_Don_t_Delete_or_Modify AS (
  SELECT
    brand_id,
    asin,
    client_id,
    ...
    fulfillment_channel,
    currency
  FROM View_of_Historical_Sales_Archive_DS_Don_t_Delete_or_Modify
  ...
  FROM _View_of_sales_by_products_archive
  GROUP BY
    product_id,
    country_id
), _Add_SDLY_Field AS (
  ...
  SELECT
    ...
    batch_id,
    batch_last_run
  FROM _View_of_amazon_order_items
  ...
  SELECT
    ...
    _View_of_sales_by_products_archive.sales AS sales_sdly,
    _View_of_sales_by_products_archive.expanded_units AS expanded_units_sdly,
    _View_of_sales_by_products_archive.units AS units_sdly
  FROM _Add_SDLY_Field
  LEFT JOIN _View_of_sales_by_products_archive
    ON _Add_SDLY_Field.sdly = _View_of_sales_by_products_archive.date
    AND _Add_SDLY_Field.product_id = _View_of_sales_by_products_archive.product_id
  ...
  SELECT
    ...
    country_id,
    date,
    CAST(expanded_units AS VARCHAR) AS expanded_units,
    CAST(sales AS VARCHAR) AS sales,
    CAST(units AS VARCHAR) AS units,
    CAST(units_sdly AS VARCHAR) AS units_sdly,
    CAST(sales_sdly AS VARCHAR) AS sales_sdly,
    CAST(expanded_units_sdly AS VARCHAR) AS expanded_units_sdly,
    CAST(cogs AS VARCHAR) AS cogs,
    CAST(max_daily_velocity AS DECIMAL(38, 0)) AS max_daily_velocity,
    amazon_seller_id,
    velocity,
    primary_velocity,
    fulfillment_channel,
    currency
  FROM _View_of_Historical_Sales_Archive_DS_Don_t_Delete_or_Modify
